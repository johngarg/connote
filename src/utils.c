...
bool generate_timestamp_now(char *dest) {
  ...
  if (strftime(dest, ID_LEN + 1, ID_FORMAT, local) == 0) {
    fprintf(stderr,
            "ERROR: Failed to format time when generating new timestamp.\n");
    return false;
  };
  return true;
}
...
bool str_append_slice(const char *src, size_t start, size_t end, char *dest,
  ...
  snprintf(dest + *current_pos, dest_size - *current_pos, "%.*s", (int)length,
           src + start);
...
bool construct_filename(char *id, char *sig, char *title, char **keywords,
  ...
  str_append_slice(id, 0, strlen(id), dest_filename, max_len_without_ext,
                   &current_pos);

  if (sig != NULL && sig[0] != '\0') {
    str_append_slice("==", 0, 2, dest_filename, max_len_without_ext,
                     &current_pos);
    str_append_slice(sig, 0, strlen(sig), dest_filename, max_len_without_ext,
                     &current_pos);
  }
  ...
  if (type == 1) {
    str_append_slice(".md", 0, 3, dest_filename, max_len_without_ext,
                     &current_pos);
    return true;
  }
...
void write_frontmatter_to_buffer(char *buffer, size_t buffer_size, char *id,
  ...
  // TITLE
  written =
      snprintf(buf_ptr, remaining_size, "title: %s\n", title ? title : "");
  buf_ptr += written;
  remaining_size -= written;
  ...
  written =
      snprintf(buf_ptr, remaining_size, "signature: %s\n", sig ? new_sig : "");
  buf_ptr += written;
  remaining_size -= written;
